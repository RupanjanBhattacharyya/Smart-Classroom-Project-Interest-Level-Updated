...
def train(t_type, model, output_path, epochs, batch_size):
    print('** LOADING DATA **')
    t_paths = np.load('data/training_paths.npy')
    t_labels = np.load('data/training_labels.npy')
    t_paths, t_labels, t_weights = process_data(t_type, t_paths, t_labels)
    v_paths = np.load('data/validation_paths.npy')
    v_labels = np.load('data/validation_labels.npy')
