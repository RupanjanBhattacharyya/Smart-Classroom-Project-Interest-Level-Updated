...
def predict_from_camera(detector):
    ...
    while True:
        ...
        boxes, scores, classes, num_detections, emotions_detected, emotion_index_detected = detector.run(image)

        # Fixed text drawing
        text1 = "Emotion Detected: {}".format(emotions_detected)
        cv2.putText(image, text1, (50, 50), cv2.FONT_HERSHEY_DUPLEX,
                    0.7, (0, 0, 255), 1)
        
        text2 = "Emotion Index: {}".format(emotion_index_detected)
        cv2.putText(image, text2, (50, 80), cv2.FONT_HERSHEY_DUPLEX,
                    0.7, (0, 0, 255), 1)
...
if __name__ == "__main__":
    if sys.argv[1] == '-c':
        ...
        if fig:
            fig.savefig('emotion_analysis.png', dpi=300, bbox_inches='tight')
    elif sys.argv[1] == '-v':
        K.clear_session()
        tf.compat.v1.reset_default_graph()
        detector = TensorflowDetector(PATH_TO_CKPT, PATH_TO_CLASS, PATH_TO_REGRESS)
